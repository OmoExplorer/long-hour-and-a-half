...
/**
 * Represents the character's urinary bladder.
 * @see Sphincter
 * @see Bladder
 *
 * @property sphincter This bladder's sphincter.
 */
class Bladder(fullness: Int, @Suppress("KDocMissingDocumentation") val sphincter: Sphincter) {
    /** Fullness level in milliliters. */
    var fullness = fullness
        set(value) {
            field = value.coerceIn(0, maximalFullnessLevel)
            ui.bladderFullnessChanged(fullness)
        }

    /** Amount of water in character's tummy in milliliters. */
    var waterInTummy = 0
        set(value) {
            field = value.coerceIn(0, 2000)
            ui.bellyWaterLevelChanged(waterInTummy)
        }


    /**
     * Fullness level where a fatal leak always occurs.
     * @see fullness
     */
    val maximalFullnessLevel get() = if (core.hardcore) MAXIMAL_FULLNESS_HARDCORE else MAXIMAL_FULLNESS

    /**
     * Fullness level where leaks begin.
     * @see fullness
     */
    val leakingFullnessLevel get() = if (core.hardcore) LEAKING_FULLNESS_HARDCORE else LEAKING_FULLNESS

    /** Produces urine for specified [duration in minutes][minutes]. */
    fun produceUrine(minutes: Int) {
        passivelyProduceUrine(minutes)
        if (waterInTummy > 0)
            produceUrineFromWater(minutes)
    }

    private fun passivelyProduceUrine(minutes: Int) {
        addFullnessWithFormula(PASSIVE_PEE_PRODUCTION_RATIO, PASSIVE_PEE_PRODUCTION_RANDOM_BONUS_CAP, minutes)
    }

    private fun produceUrineFromWater(minutes: Int) {
        val addedFullness =
                addFullnessWithFormula(WATER_PEE_PRODUCTION_RATIO, WATER_PEE_PRODUCTION_RANDOM_BONUS_CAP, minutes)
        waterInTummy -= addedFullness
    }

    private fun addFullnessWithFormula(ratio: Double, bonusCap: Int, minutes: Int): Int {
        val fullnessToAdd = (ratio * minutes).roundToInt() + random.nextInt(bonusCap)
        fullness += fullnessToAdd
        return fullnessToAdd
    }

    companion object {
        private const val MAXIMAL_FULLNESS = 1300
        private const val MAXIMAL_FULLNESS_HARDCORE = 1100

        private const val LEAKING_FULLNESS = 850
        private const val LEAKING_FULLNESS_HARDCORE = 700

        private const val PASSIVE_PEE_PRODUCTION_RATIO = 1.25
        private const val PASSIVE_PEE_PRODUCTION_RANDOM_BONUS_CAP = 3

        private const val WATER_PEE_PRODUCTION_RATIO = 3.0
        private const val WATER_PEE_PRODUCTION_RANDOM_BONUS_CAP = 6

        /** Maximal value of random fullness at start. */
        const val RANDOM_FULLNESS_CAP = 700
    }
}